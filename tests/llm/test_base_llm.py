...
def test_distance_calculation(agent, capfd):
    ...
    # 0.3972871020812553 km
    # Observation: La distance entre Meriadeck et Place Gambetta est de 0.32 km
    # 0.3972871020812553 / 15 * 60 = 1.58 minutes

    # La distance entre Meriadeck et la Place Gambetta est de 0.3972871020812553
    # km. Si tu roules à 15 km/h, le temps de trajet sera de 1.59 minutes.
    ...
    assert "1.59" in response["output"].lower() or "1 minute" in response["output"].lower()
    assert "meriadeck" in response["output"].lower()
