...
def plot_station_activity(
    ...
    for feature in features_to_plot:
        trace = go.Scatter(x=temp[date_col], y=temp[feature], mode="lines", name=feature)
...
def plot_profile_station(data, station_id, feature_to_plot, aggfunc="mean", filter_data=True, vmin=None):
    """
    ...
    pivot_station = station_resample.pivot_table(
        index=["weekday"], columns=["hours"], values=feature_to_plot, aggfunc=aggfunc
    )

    plt.subplots(figsize=(20, 5))
    sns.heatmap(pivot_station, linewidths=0.5, cmap="coolwarm", vmin=vmin, annot=True)
    plt.title(
        "Profile d'activité de la station N°"
        + str(station_id)
        + " / "
        + feature_to_plot
        + " (aggrégation : "
...
def plot_station_anomalies(
    ...
    # Axe 1
    trace = go.Scatter(
        x=data_pred["date"], y=data_pred["available_bikes"], mode="lines", line={"width": 2}, name="Vélo disponible"
    )
    data_graph.append(trace)
    ...
    trace_ano = go.Scatter(
        ...
        mode="lines",
        line={"width": 1, "dash": "dot", "color": "rgba(189,189,189,1)"},
        yaxis="y2",
...
def plot_map_station_with_plotly(
    station_control,
    station_id=None,
    width=800,
    height=600,
    ...
    if station_id is not None:
        ...
        center_lat = station_control[station_control["station_id"] == station_id]["lat"].values[0]
        center_lon = station_control[station_control["station_id"] == station_id]["lon"].values[0]
        zoom_plot = 15
    else:
    ...
    # Non monitoré
    station_control.loc[station_control["mean_activity"] < THRESHOLD_PROFILE_STATION, "etat"] = "non surveillée"
    ...
    try:
        station_control["anomaly_since_str"] = station_control["anomaly_since"].dt.strftime(
            date_format="%Y-%m-%d %H:%M"
        )
    except KeyError:
        # Pour vcub_watcher intégration
        # https://github.com/armgilles/vcub_keeper/issues/49#issuecomment-822504771
        station_control["anomaly_since_str"] = station_control["En anomalie depuis"].dt.strftime(
            date_format="%Y-%m-%d %H:%M"
        )
    ...
    # Color for etat
    color_etat = {"anomaly": "#EB4D50", "inactive": "#5E9BE6", "normal": "#6DDE75", "non surveillée": "#696A6A"}
    ...
    fig.update_layout(
        mapbox=dict(
            center=dict(lat=center_lat, lon=center_lon), accesstoken=MAPBOX_TOKEN, zoom=zoom_plot, style="light"
        ),
        showlegend=True,
        width=width,
        height=height,
...
def plot_map_station_with_kepler(station_control, station_id=None):
    ...
    config_global = {
        ...
        "config": {
            "visState": {
                ...
                "layers": [
                    {
                        ...
                        "config": {
                            ...
                            "visConfig": {
                                ...
                                "colorRange": {
                                    ...
                                    "type": "custom",
                                    "category": "Custom",
                                    "colors": ["#696A6A", "#6DDE75", "#EB4D50", "#5E9BE6"],
                                },
                                "strokeColorRange": {
                                    "name": "Global Warming",
                                    "type": "sequential",
                        ...
                        "visualChannels": {
                            "colorField": {"name": "etat_id_sort", "type": "integer"},
                            "colorScale": "ordinal",
                ...
                "interactionConfig": {
                    "tooltip": {
                        "fieldsToShow": {
                            "data_1": [
                                {"name": "station_id", "format": None},
                                {"name": "NOM", "format": None},
                                {"name": "etat", "format": None},
                                {"name": "available_bikes", "format": None},
                                {"name": "anomaly_since_str", "format": None},
                            ]
                        },
                        "compareMode": True,
            ...
            "mapStyle": {
                ...
                "threeDBuildingColor": [137, 137, 137],
                "mapStyles": {},
            },
        },
    }
    ...
    fake_station_every_etat = [
        [990, 0.4, 1, 0, 35, 60.9616624, -39.1527227, "Station fake anomaly", "anomaly", "2021-04-07 09:00"],
        [991, 0.42, 1, 0, 21, 60.9616624, -39.1527227, "Station fake normal", "normal", "-"],
        [992, 0.36, 0, 1, 22, 60.9616624, -39.1527227, "Station fake inactive", "inactive", "-"],
        [993, 0.0, 0, 0, 22, 60.9616624, -39.1527227, "Station non surveillée", "non surveillée", "-"],
    ]
    ...
    etat_id_sort = {"non surveillée": 0, "normal": 1, "anomaly": 2, "inactive": 3}
    ...
    # To remove message in kepler
    # Message serialization failed with:
    # Out of range float values are not JSON compliant
    station_control = station_control.fillna("")
...
def plot_station_anomalies_with_score(
    ...
    layout = dict(
        ...
        yaxis={
            "title": "Nombre de vélo disponible",
            "title_font": {"color": "rgba(100, 111, 251, 1)"},
            "tickfont": {"color": "rgba(100, 111, 251, 1)"},
        },
        yaxis2={
            "title": "Absence consécutive de prise de vélo",
        ...
        xaxis2=dict(rangeslider=dict(visible=False), type="date", hoverformat="%a %Y-%m-%d %H:%M"),
        yaxis3={
            "title": "Score",
